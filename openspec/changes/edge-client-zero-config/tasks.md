...
- [ ] 设置向导 API 端点
  - [ ] `POST /api/setup/validate-server` - 验证 API 服务器
  - [ ] `POST /api/setup/login` - 用户登录
  - [ ] `GET /api/setup/devices` - 获取设备列表
  - [ ] `POST /api/setup/devices` - 创建新设备
  - [ ] `GET /api/setup/detect-sources` - 检测输入源
  - [ ] `POST /api/setup/save-config` - 保存配置
  - [ ] `POST /api/setup/start` - 启动推流
...
- [ ] 输入源自动检测
  - [ ] V4L2 设备检测（Linux）
  - [ ] AVFoundation 设备检测（macOS）
  - [ ] DirectShow 设备检测（Windows）
  - [ ] 返回设备列表和元数据
