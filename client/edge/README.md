...
### 🚀 零配置启动
- ✅ **3步设置向导**（API → 登录 → 设备）- 小白用户友好
- ✅ **自动检测输入源**（摄像头优先，无摄像头时使用测试源）
- ✅ **智能默认配置**（GStreamer 引擎，自动优化参数）
- ✅ **无需手动编辑配置文件**

### 📹 视频采集与推流
- ✅ 多种视频输入源支持（V4L2、RTSP、文件、测试画面）
- ✅ 双引擎支持（GStreamer、FFmpeg）
- ✅ RTSP 推流到 MediaMTX，带设备密钥认证
- ✅ 与 PawStream API 服务器集成
...
#    ③ 选择或创建设备
#
# 4. 完成！客户端自动检测摄像头并开始推流
```

**自动智能配置**：
- 🔍 **自动检测输入源**：优先使用摄像头，无摄像头时使用测试源
- 🎬 **默认引擎**：GStreamer（低延迟、高性能）
- ⚙️ **自动优化参数**：无需手动调整
